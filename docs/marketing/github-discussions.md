# GitHub Discussions Templates

## Welcome to the SynapseMD Community!

This is the place to discuss the **SynapseMD** project - a personal health management system based on Claude Code.

## Quick Links

- 📖 [Documentation](https://github.com/huifer/SynapseMD#readme)
- 🐛 [Report Issues](https://github.com/huifer/SynapseMD/issues)
- 💡 [Feature Requests](https://github.com/huifer/SynapseMD/issues?q=is%3Aissue+is%3Aopen+label%3Aenhancement)
- 📝 [Contribution Guide](https://github.com/huifer/SynapseMD/blob/main/CONTRIBUTING.md)

## Discussion Categories

### 💬 General Discussion (General)
Any questions, ideas, or suggestions about the project

### 🎯 Feature Requests (Feature Requests)
Features you want, improvement suggestions

### 🐛 Help & Support (Help & Support)
Running into issues? Ask here

### 📚 Show & Tell (Show & Tell)
Show how you use this project

### 🌍 Internationalization (Internationalization)
Multi-language support discussions

## Frequently Asked Questions

### Q: Is this project officially from Anthropic?
**A:** No. This is an independent open-source project with no affiliation with Anthropic or Claude.ai.

### Q: Will my data be uploaded to the cloud?
**A:** No. All data is stored on your local file system.

### Q: Do I need a Python/Node.js environment?
**A:** No. You only need to install Claude Code CLI.

### Q: Can it be used for commercial purposes?
**A:** Yes, the project uses the MIT license.

## Community Guidelines

1. **Respect others** - Stay friendly and constructive
2. **Stay on topic** - Keep discussions relevant to the project
3. **Search first** - Search for similar discussions before posting
4. **Privacy awareness** - Do not share real medical data in discussions

## Ways to Contribute

We welcome all forms of contribution:

- 🐛 Report bugs
- 💡 Propose new features
- 📝 Improve documentation
- 🔧 Submit code
- 🌍 Help with translation
- 💬 Answer community questions

See the [Contribution Guide](https://github.com/huifer/SynapseMD/blob/main/CONTRIBUTING.md) for more information.

---

Have questions? Feel free to start a new discussion!
